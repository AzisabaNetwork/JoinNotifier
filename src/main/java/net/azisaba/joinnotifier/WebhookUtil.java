...
public class WebhookUtil {
    ...
    public static void sendDiscordWebhook(String configPath, String username, String content) {
        try {
            String s = JoinNotifier.getInstance().getConfig().getString(configPath);
            if (s == null || s.isEmpty()) {
                return;
            }
            ...
            con.setRequestProperty("User-Agent", "JoinNotifier/" + JoinNotifier.getInstance().getDescription().getVersion());
            ...
            if (errorStream != null) {
                ...
                JoinNotifier.getInstance().getLogger().warning("Discord webhook returned " + con.getResponseCode() + ": " + err);
            }
